...
from cybernetics.lib.telemetry import (
    MAX_BATCH_SIZE,
    MAX_QUEUE_SIZE,
    Telemetry,
...
from cybernetics.types.generic_event import GenericEvent
from cybernetics.types.session_end_event import SessionEndEvent
from cybernetics.types.session_start_event import SessionStartEvent
from cybernetics.types.telemetry_batch import TelemetryBatch
from cybernetics.types.telemetry_event import TelemetryEvent
from cybernetics.types.telemetry_response import TelemetryResponse
from cybernetics.types.unhandled_exception_event import UnhandledExceptionEvent
...
class MockAsyncCyberneticsProvider:
    def __init__(self):
        self.loop = Mock()
        self.callbacks = []
...
class TestTelemetryClass:
    def setup_method(self):
        self.worldlines_provider = MockAsyncCyberneticsProvider()
        self.telemetry = Telemetry(self.worldlines_provider, session_id="test-session-id")
    ...
    async def test_initialization(self):
        self.worldlines_provider.execute_callbacks()
    ...
    def test_log_queue_full(self):
        ...
        with patch("cybernetics.lib.telemetry.logger") as mock_logger:
            result = self.telemetry._log(event1, event2)
...
class TestTelemetryEnvironment:
    ...
    def test_is_telemetry_enabled(self, env_value, expected):
        with patch.dict(os.environ, {"CYBERNETICS_TELEMETRY": env_value}):
    ...
    def test_init_telemetry_enabled(self):
        with patch.dict(os.environ, {"CYBERNETICS_TELEMETRY": "1"}):
            worldlines_provider = MockAsyncCyberneticsProvider()
            telemetry = init_telemetry(worldlines_provider, session_id="test-session-id")
            assert telemetry is not None
            assert isinstance(telemetry, Telemetry)
            telemetry.stop()

    def test_init_telemetry_disabled(self):
        with patch.dict(os.environ, {"CYBERNETICS_TELEMETRY": "0"}):
            worldlines_provider = MockAsyncCyberneticsProvider()
            telemetry = init_telemetry(worldlines_provider, session_id="test-session-id")
    ...
    def test_init_telemetry_with_exception(self):
        with patch.dict(os.environ, {"CYBERNETICS_TELEMETRY": "1"}):
            worldlines_provider = Mock()
            worldlines_provider.get_loop.side_effect = Exception("Init error")
            with patch("cybernetics.lib.telemetry.logger") as mock_logger:
                telemetry = init_telemetry(worldlines_provider, session_id="test-session-id")
            assert telemetry is None
...
class TestTelemetryFlush:
    ...
    def test_unsupported_telemetry_endpoint_disables_without_warning_retry(self):
        request = httpx.Request("POST", "https://example.com/api/v1/telemetry")
        response = httpx.Response(404, request=request)
        error = APIStatusError(
            "Route POST:/api/v1/telemetry not found",
            response=response,
            body={"message": "not found"},
        )

        with (
            patch.object(self.telemetry, "_send_batch", new_callable=AsyncMock, side_effect=error)
            as mock_send,
            patch("cybernetics.lib.telemetry.logger") as mock_logger,
        ):
            asyncio.run(self.telemetry._flush())

        assert mock_send.call_count == 1
        assert self.telemetry._enabled is False
        assert len(self.telemetry._queue) == 0
        assert asyncio.run(self.telemetry._wait_until_drained()) is True
        assert self.telemetry._log(self.telemetry._session_end_event()) is False
        mock_logger.warning.assert_not_called()

    @pytest.mark.asyncio
    async def test_log_exception_sync_from_event_loop_protection(self):
        with patch("cybernetics.lib.telemetry._current_loop") as mock_current_loop:
...
class TestSyncContextManager:
    ...
    async def test_send_batch_uses_sync_context_manager(self):
        worldlines_provider = MockAsyncCyberneticsProvider()
        telemetry = Telemetry(worldlines_provider, session_id="test-session-id")
        ...
        worldlines_provider._client_cm.__enter__.assert_called_once()
        worldlines_provider._client_cm.__exit__.assert_called_once()
        worldlines_provider.telemetry_send_mock.assert_called_once()
...
class TestCrossLoopSafety:
    ...
    async def test_trigger_flush_from_different_loop(self):
        worldlines_provider = MockAsyncCyberneticsProvider()
        telemetry = Telemetry(worldlines_provider, session_id="test-session-id")
        worldlines_provider.execute_callbacks()
