...
from cybernetics._exceptions import APIError, APIStatusError, RequestFailedError
...
from cybernetics.types.severity import Severity
...
from cybernetics.types.telemetry_send_request import TelemetrySendRequest
from cybernetics.types.unhandled_exception_event import UnhandledExceptionEvent

from .async_worldlines_provider import AsyncCyberneticsProvider
...
class Telemetry:
    def __init__(self, worldlines_provider: AsyncCyberneticsProvider, session_id: str):
        self._worldlines_provider: AsyncCyberneticsProvider = worldlines_provider
    ...
    def _start(self):
        def cb():
            ...
            self._task = asyncio.create_task(self._periodic_flush(), name="worldlines-telemetry")

        _ = self._worldlines_provider.get_loop().call_soon_threadsafe(cb)
    ...
    def _trigger_flush(self):
        if self._enabled and self._flush_event:
            _ = self._worldlines_provider.get_loop().call_soon_threadsafe(self._flush_event.set)
    ...
    def _wait_until_drained_sync(self) -> bool:
        try:
            return asyncio.run_coroutine_threadsafe(
                self._wait_until_drained(), self._worldlines_provider.get_loop()
    ...
    async def _send_batch_with_retry(self, batch: TelemetryBatch) -> TelemetryResponse:
        while True:
            try:
                ...
                if _is_unsupported_telemetry_error(e):
                    self._disable()
                    logger.debug("Telemetry endpoint is not available; disabling telemetry")
    ...
    async def _send_batch(self, batch: TelemetryBatch) -> TelemetryResponse:
        with self._worldlines_provider.aclient(ClientConnectionPoolType.TELEMETRY) as client:
    ...
    def _disable(self) -> None:
        ...
        with self._queue_lock:
            self._queue.clear()
        with self._counter_lock:
            self._flush_counter = self._push_counter
...
def _is_telemetry_enabled() -> bool:
    return os.environ.get("CYBERNETICS_TELEMETRY", "1").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _is_unsupported_telemetry_error(error: APIError) -> bool:
    return isinstance(error, APIStatusError) and error.status_code in {404, 405, 501}


def init_telemetry(
    worldlines_provider: AsyncCyberneticsProvider, session_id: str
) -> Telemetry | None:
    try:
        return Telemetry(worldlines_provider, session_id) if _is_telemetry_enabled() else None
    except Exception as e:
