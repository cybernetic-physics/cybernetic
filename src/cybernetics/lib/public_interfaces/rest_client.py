"""RestClient for Cybernetics API REST operations."""
...
from cybernetics._types import NoneType
...
class RestClient(TelemetryProvider):
    ...
    - publish_checkpoint_from_worldlines_path() - publish a checkpoint to make it public
    - unpublish_checkpoint_from_worldlines_path() - unpublish a checkpoint to make it private
    - set_checkpoint_ttl_from_worldlines_path() - set or remove TTL on a checkpoint
    ...
    @staticmethod
    def _checkpoint_route_from_worldlines_path(
        worldlines_path: str,
        weights_info: types.WeightsInfoResponse | None = None,
    ) -> tuple[types.ModelID, str]:
        parsed = types.ParsedCheckpointCyberneticsPath.from_worldlines_path(worldlines_path)
        training_run_id = (
            weights_info.training_run_id
            if weights_info is not None and weights_info.training_run_id
            else parsed.training_run_id
        )
        return training_run_id, parsed.checkpoint_id

    async def _checkpoint_route_from_path_async(
        self,
        worldlines_path: str,
    ) -> tuple[types.ModelID, str]:
        weights_info = await self.get_weights_info_by_worldlines_path_async(worldlines_path)
        return self._checkpoint_route_from_worldlines_path(worldlines_path, weights_info)
    ...
    def _get_training_run_by_worldlines_path_submit(
        self,
        worldlines_path: str,
        access_scope: Literal["owned", "accessible"] = "owned",
    ) -> AwaitableConcurrentFuture[types.TrainingRun]:
        """Internal method to submit training-run lookup by checkpoint path."""

        async def _get_training_run_by_worldlines_path_async() -> types.TrainingRun:
            training_run_id, _ = await self._checkpoint_route_from_path_async(worldlines_path)
            return await self._get_training_run_submit(
                training_run_id,
                access_scope=access_scope,
            )

        return self.holder.run_coroutine_threadsafe(_get_training_run_by_worldlines_path_async())

    @sync_only
    @capture_exceptions(fatal=True)
    def get_training_run_by_worldlines_path(
        self, worldlines_path: str, access_scope: Literal["owned", "accessible"] = "owned"
    ) -> ConcurrentFuture[types.TrainingRun]:
        """Get training run info.

        Args:
        - `worldlines_path`: The worldlines path to the checkpoint
        ...
        Example:
        ```python
        future = rest_client.get_training_run_by_worldlines_path("worldlines://run-id/weights/checkpoint-001")
        response = future.result()
        print(f"Training Run ID: {response.training_run_id}, Base: {response.base_model}")
        ```
        """
        return self._get_training_run_by_worldlines_path_submit(
            worldlines_path,
            access_scope=access_scope,
        ).future()

    @capture_exceptions(fatal=True)
    async def get_training_run_by_worldlines_path_async(
        self, worldlines_path: str, access_scope: Literal["owned", "accessible"] = "owned"
    ) -> types.TrainingRun:
        """Async version of get_training_run_by_worldlines_path."""
        return await self._get_training_run_by_worldlines_path_submit(
            worldlines_path,
            access_scope=access_scope,
        )

    async def _get_weights_info_by_worldlines_path_async_impl(
        self,
        worldlines_path: str,
        ...
        async def _send_request() -> types.WeightsInfoResponse:
            with self.holder.aclient(ClientConnectionPoolType.TRAIN) as client:
                return await client.post(
                    "/api/v1/weights_info",
                    body={"worldlines_path": worldlines_path},
    ...
    @capture_exceptions(fatal=True)
    def get_weights_info_by_worldlines_path(
        self, worldlines_path: str
    ) -> APIFuture[types.WeightsInfoResponse]:
        """Get checkpoint information from a worldlines path.
        ...
        Example:
        ```python
        future = rest_client.get_weights_info_by_worldlines_path("worldlines://run-id/weights/checkpoint-001")
        ...
        return self.holder.run_coroutine_threadsafe(
            self._get_weights_info_by_worldlines_path_async_impl(worldlines_path)
        )

    @capture_exceptions(fatal=True)
    async def get_weights_info_by_worldlines_path_async(
        self, worldlines_path: str
    ) -> types.WeightsInfoResponse:
        """Async version of get_weights_info_by_worldlines_path."""
        return await self._get_weights_info_by_worldlines_path_async_impl(worldlines_path)
    ...
    @sync_only
    @capture_exceptions(fatal=True)
    def delete_checkpoint_from_worldlines_path(
        self, worldlines_path: str
    ) -> ConcurrentFuture[None]:
        """Delete a checkpoint referenced by a worldlines path."""

        async def _delete_by_path_async() -> None:
            training_run_id, checkpoint_id = await self._checkpoint_route_from_path_async(
                worldlines_path
            )
            await self._delete_checkpoint_submit(training_run_id, checkpoint_id)

        return self.holder.run_coroutine_threadsafe(_delete_by_path_async()).future()

    @capture_exceptions(fatal=True)
    async def delete_checkpoint_from_worldlines_path_async(self, worldlines_path: str) -> None:
        """Async version of delete_checkpoint_from_worldlines_path."""
    ...
    @sync_only
    @capture_exceptions(fatal=True)
    def get_checkpoint_archive_url_from_worldlines_path(
        self, worldlines_path: str
    ) -> ConcurrentFuture[types.CheckpointArchiveUrlResponse]:
        """Get signed URL to download checkpoint archive.
        ...
        - A `Future` containing the `CheckpointArchiveUrlResponse` with signed URL and expiration
        """
        async def _get_archive_by_path_async() -> types.CheckpointArchiveUrlResponse:
            training_run_id, checkpoint_id = await self._checkpoint_route_from_path_async(
                worldlines_path
            )
            return await self._get_checkpoint_archive_url_submit(training_run_id, checkpoint_id)

        return self.holder.run_coroutine_threadsafe(_get_archive_by_path_async()).future()

    @capture_exceptions(fatal=True)
    async def get_checkpoint_archive_url_from_worldlines_path_async(
        self, worldlines_path: str
    ) -> types.CheckpointArchiveUrlResponse:
        """Async version of get_checkpoint_archive_url_from_worldlines_path."""
        training_run_id, checkpoint_id = await self._checkpoint_route_from_path_async(
            worldlines_path
        )
        return await self._get_checkpoint_archive_url_submit(training_run_id, checkpoint_id)
    ...
    @sync_only
    @capture_exceptions(fatal=True)
    def publish_checkpoint_from_worldlines_path(
        self, worldlines_path: str
    ) -> ConcurrentFuture[None]:
        """Publish a checkpoint referenced by a worldlines path to make it publicly accessible.

        Only the exact owner of the training run can publish checkpoints.
        Published checkpoints can be unpublished using the unpublish_checkpoint_from_worldlines_path method.

        Args:
        - `worldlines_path`: The worldlines path to the checkpoint (e.g., "worldlines://run-id/weights/0001")
        ...
        Example:
        ```python
        future = rest_client.publish_checkpoint_from_worldlines_path("worldlines://run-id/weights/0001")
        future.result()  # Wait for completion
        print("Checkpoint published successfully")
        ```
        """
        async def _publish_by_path_async() -> None:
            training_run_id, checkpoint_id = await self._checkpoint_route_from_path_async(
                worldlines_path
            )
            await self._publish_checkpoint_submit(training_run_id, checkpoint_id)

        return self.holder.run_coroutine_threadsafe(_publish_by_path_async()).future()

    @capture_exceptions(fatal=True)
    async def publish_checkpoint_from_worldlines_path_async(self, worldlines_path: str) -> None:
        """Async version of publish_checkpoint_from_worldlines_path."""
        training_run_id, checkpoint_id = await self._checkpoint_route_from_path_async(
            worldlines_path
        )
        await self._publish_checkpoint_submit(training_run_id, checkpoint_id)
    ...
    @sync_only
    @capture_exceptions(fatal=True)
    def unpublish_checkpoint_from_worldlines_path(
        self, worldlines_path: str
    ) -> ConcurrentFuture[None]:
        """Unpublish a checkpoint referenced by a worldlines path to make it private again.
        ...
        Example:
        ```python
        future = rest_client.unpublish_checkpoint_from_worldlines_path("worldlines://run-id/weights/0001")
        future.result()  # Wait for completion
        print("Checkpoint unpublished successfully")
        ```
        """
        async def _unpublish_by_path_async() -> None:
            training_run_id, checkpoint_id = await self._checkpoint_route_from_path_async(
                worldlines_path
            )
            await self._unpublish_checkpoint_submit(training_run_id, checkpoint_id)

        return self.holder.run_coroutine_threadsafe(_unpublish_by_path_async()).future()

    @capture_exceptions(fatal=True)
    async def unpublish_checkpoint_from_worldlines_path_async(self, worldlines_path: str) -> None:
        """Async version of unpublish_checkpoint_from_worldlines_path."""
        training_run_id, checkpoint_id = await self._checkpoint_route_from_path_async(
            worldlines_path
        )
        await self._unpublish_checkpoint_submit(training_run_id, checkpoint_id)
    ...
    @sync_only
    @capture_exceptions(fatal=True)
    def set_checkpoint_ttl_from_worldlines_path(
        self, worldlines_path: str, ttl_seconds: int | None
    ) -> ConcurrentFuture[None]:
        """Set or remove the TTL on a checkpoint referenced by a worldlines path.
        ...
        Example:
        ```python
        future = rest_client.set_checkpoint_ttl_from_worldlines_path("worldlines://run-id/weights/0001", 86400)
        future.result()  # Wait for completion
        print("Checkpoint TTL set successfully")
        ```
        """
        async def _set_ttl_by_path_async() -> None:
            training_run_id, checkpoint_id = await self._checkpoint_route_from_path_async(
                worldlines_path
            )
            await self._set_checkpoint_ttl_submit(training_run_id, checkpoint_id, ttl_seconds)

        return self.holder.run_coroutine_threadsafe(_set_ttl_by_path_async()).future()

    @capture_exceptions(fatal=True)
    async def set_checkpoint_ttl_from_worldlines_path_async(
        self, worldlines_path: str, ttl_seconds: int | None
    ) -> None:
        """Async version of set_checkpoint_ttl_from_worldlines_path."""
        training_run_id, checkpoint_id = await self._checkpoint_route_from_path_async(
            worldlines_path
        )
        await self._set_checkpoint_ttl_submit(training_run_id, checkpoint_id, ttl_seconds)
    ...
    def _cancel_session_submit(
        self,
        session_id: str,
    ) -> AwaitableConcurrentFuture[types.WorldlinesSessionCancelResponse]:
        """Internal method to submit a Worldlines session cancel request."""

        async def _cancel_session_async() -> types.WorldlinesSessionCancelResponse:
            async def _send_request() -> types.WorldlinesSessionCancelResponse:
                with self.holder.aclient(ClientConnectionPoolType.TRAIN) as client:
                    return await client.post(
                        f"/v1/worldlines/sessions/{session_id}/cancel",
                        body={},
                        cast_to=types.WorldlinesSessionCancelResponse,
                    )

            return await self.holder.execute_with_retries(_send_request)

        return self.holder.run_coroutine_threadsafe(_cancel_session_async())

    @sync_only
    @capture_exceptions(fatal=True)
    def cancel_session(self, session_id: str) -> ConcurrentFuture[types.WorldlinesSessionCancelResponse]:
        """Cancel a Worldlines session and request safe cleanup for its compute lease.

        The control plane marks session futures terminal and asks the autoscaler to
        stop session-owned leases only when no other live session or active future
        still references them.
        """
        return self._cancel_session_submit(session_id).future()

    @capture_exceptions(fatal=True)
    async def cancel_session_async(self, session_id: str) -> types.WorldlinesSessionCancelResponse:
        """Async version of cancel_session."""
        return await self._cancel_session_submit(session_id)
