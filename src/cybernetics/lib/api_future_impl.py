...
from cybernetics._exceptions import RequestFailedError
from cybernetics.lib.client_connection_pool_type import ClientConnectionPoolType
from cybernetics.lib.public_interfaces.api_future import APIFuture
from cybernetics.lib.telemetry import Telemetry, is_user_error
from cybernetics.types import RequestErrorCategory
from cybernetics.types.future_retrieve_request import FutureRetrieveRequest
...
if TYPE_CHECKING:
    from cybernetics.lib.internal_client_holder import InternalClientHolder
    ...
    async def _result_async(self, timeout: float | None = None) -> T:
        ...
        async with contextlib.AsyncExitStack() as stack:
            while True:
                ...
                headers = {
                    "X-Cybernetics-Request-Iteration": str(iteration),
                    "X-Cybernetics-Request-Type": self.request_type,
                }
                if iteration == 0:
                    headers["X-Cybernetics-Create-Promise-Roundtrip-Time"] = str(
                ...
                except cybernetics.APIConnectionError as e:
                    if telemetry := self.get_telemetry():
                        current_time = time.time()
                        telemetry.log(
