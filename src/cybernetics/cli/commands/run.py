...
This module implements the 'cybernetics run' commands, including:
...
if TYPE_CHECKING:
    from cybernetics.types import TrainingRun
...
AVAILABLE_COLUMNS: Dict[str, tuple[str, Callable[["TrainingRun"], str], str]] = {
    ...
    "lora": (
        "LoRA",
        lambda r: (
            f"Rank {r.lora_rank}" if r.is_lora and r.lora_rank else ("Yes" if r.is_lora else "No")
        ),
...
class RunListOutput(OutputBase):
    """Output for 'cybernetics run list' command."""
...
class RunInfoOutput(OutputBase):
    """Output for 'cybernetics run info' command."""
    ...
    def get_table_rows(self) -> List[List[str]]:
        ...
        if self.run.last_checkpoint:
            ...
            rows.append(["  - Path", self.run.last_checkpoint.worldlines_path])
        ...
        if self.run.last_sampler_checkpoint:
            ...
            rows.append(["  - Path", self.run.last_sampler_checkpoint.worldlines_path])
