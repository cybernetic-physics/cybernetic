...
This module implements the 'worldlines checkpoint' commands, including:
...
if TYPE_CHECKING:
    ...
    from cybernetics.types import Checkpoint, TrainingRun
...
class CheckpointListOutput(OutputBase):
    """Output for 'worldlines checkpoint list' command."""
...
class CheckpointInfoOutput(OutputBase):
    """Output for 'worldlines checkpoint info' command."""
    ...
    def get_table_rows(self) -> List[List[str]]:
        ...
        rows = [
            ...
            ["Cybernetics Path", self.checkpoint.worldlines_path],
        ]
        ...
        if self.checkpoint.worldlines_path.startswith("worldlines://"):
            parts = self.checkpoint.worldlines_path.replace("worldlines://", "").split("/")
            if parts:
...
class CheckpointDownloadOutput(OutputBase):
    """Output for 'worldlines checkpoint download' command."""
    ...
    def __init__(
        ...
        Args:
            checkpoint_path: The worldlines path to the checkpoint
...
class CheckpointHubUploadOutput(OutputBase):
    """Output for 'worldlines checkpoint push-hf' command."""
...
def get_checkpoint_from_path(client: "RestClient", checkpoint_path: str) -> "Checkpoint":
    """Get checkpoint details from a worldlines path.

    Args:
        checkpoint_path: A worldlines path like "worldlines://run-id/weights/0001"
    ...
    Raises:
        CyberneticsCliError: If the checkpoint cannot be retrieved
    """
    # Lazy import
    from cybernetics import ParsedCheckpointCyberneticsPath

    try:
        parsed = ParsedCheckpointCyberneticsPath.from_worldlines_path(checkpoint_path)
        weights_info = client.get_weights_info_by_worldlines_path(checkpoint_path).result()
        training_run_id = weights_info.training_run_id or parsed.training_run_id

        # Get the checkpoint info
        checkpoints_response = client.list_checkpoints(training_run_id).result()

        # Find the matching checkpoint
        for ckpt in checkpoints_response.checkpoints:
            if ckpt.worldlines_path == checkpoint_path:
                return ckpt

        raise CyberneticsCliError(f"Checkpoint not found: {checkpoint_path}")

    except ValueError as e:
        raise CyberneticsCliError(
            f"Invalid checkpoint path: {e}",
            "Checkpoint paths should be in the format: worldlines://run-id/weights/0001",
        )
    except CyberneticsCliError:
        # Re-raise our own errors
        raise
    except Exception as e:
        raise CyberneticsCliError(f"Failed to retrieve checkpoint: {e}")
...
def _export_checkpoint_to_hub(
    ...
    *,
    ...
    # Validate worldlines path
    parsed_worldlines_path = ParsedCheckpointCyberneticsPath.from_worldlines_path(worldlines_path)

    api = HfApi()
    try:
        api.whoami()
    except Exception as exc:
        raise CyberneticsCliError("Not logged in", "Run: hf auth login") from exc
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        url_response = client.get_checkpoint_archive_url_from_worldlines_path(
            worldlines_path
        ).result()
        ...
        train_unembed = None
        try:
            weights_info = client.get_weights_info_by_worldlines_path(worldlines_path).result()
            ...
            # Metadata enrichment is optional; archive validation does not depend on it.
            pass
        ...
        try:
            ...
            # Preserve the downloaded archive even if its optional config patch fails.
            pass

        if repo_id is None:
            base_short = base_model.split("/")[-1] if base_model != "unknown" else "adapter"
            derived = f"worldlines-{base_short}-{parsed_worldlines_path.training_run_id}"
            repo_id = _sanitize_repo_name(derived)
            if revision is None:
                revision = _sanitize_repo_name(
                    parsed_worldlines_path.checkpoint_id.replace("/", "-")
                )

        readme_path = extract_dir / "README.md"
        if add_model_card and not readme_path.exists():
            tags: List[str] = ["cybernetics", "peft", "lora"]
            ...
            model_card.append(f"worldlines_path: {worldlines_path}")
            model_card.extend(
                [
                    "---",
                    "",
                    "# Cybernetics LoRA Adapter",
                    "",
                    "This repository contains a LoRA adapter exported from Cybernetics.",
                    "",
                    "## Usage",
                    "",
                    "```python",
        ...
        def _readme_worldlines_path() -> str | None:
            try:
            ...
            try:
                ...
                return None
            match = re.search(r"worldlines://[^\s`]+", text)
            return match.group(0) if match else None

        existing_worldlines_path = _readme_worldlines_path()
        if existing_worldlines_path and existing_worldlines_path != worldlines_path:
            raise CyberneticsCliError(
                "Repo ID appears to contain a different Cybernetics checkpoint.",
                f"Found {existing_worldlines_path}, expected {worldlines_path}.",
            )
...
def info(cli_context: CLIContext, checkpoint_path: str) -> None:
    ...
    CHECKPOINT_PATH must be a worldlines path (e.g., worldlines://run-id/weights/0001).
    """
    # Get format from context object
    format = cli_context.format

    # Validate it's a worldlines path
    if not checkpoint_path.startswith("worldlines://"):
        raise CyberneticsCliError(
            f"Invalid checkpoint path: {checkpoint_path}",
            "Checkpoint path must be in the format: worldlines://run-id/weights/0001",
        )
    ...
    training_run = client.get_training_run_by_worldlines_path(checkpoint_path).result()
...
def publish(cli_context: CLIContext, checkpoint_path: str) -> None:
    ...
    client = create_rest_client()
    client.publish_checkpoint_from_worldlines_path(checkpoint_path).result()
...
def unpublish(cli_context: CLIContext, checkpoint_path: str) -> None:
    ...
    client = create_rest_client()
    client.unpublish_checkpoint_from_worldlines_path(checkpoint_path).result()
...
def set_ttl(cli_context: CLIContext, checkpoint_path: str, ttl: int | None, remove: bool) -> None:
    ...
    client = create_rest_client()
    client.set_checkpoint_ttl_from_worldlines_path(checkpoint_path, ttl_seconds).result()
...
def _confirm_deletion(paths: "List[str]", checkpoints: "List[Checkpoint] | None" = None) -> bool:
    ...
    if checkpoints is not None:
        ...
        for ckpt in checkpoints:
            ...
            click.echo(f"  - {ckpt.worldlines_path} ({size_str}, created {time_str})")
...
def _delete_one(client: "RestClient", path: str) -> "tuple[str, str] | None":
    ...
    try:
        client.delete_checkpoint_from_worldlines_path(path).result()
        return None
    except Exception as e:
        return (path, str(e))
...
def _delete_paths(
    ...
    """Delete a list of worldlines paths concurrently and print results."""
            ...
            json.dumps(
                {
                    ...
                    "failed": [{"worldlines_path": p, "error": e} for p, e in failed],
                }
            )
        )
    else:
...
def delete(
    ...
    Delete by explicit paths:
        ...
        worldlines checkpoint delete worldlines://run-id/weights/0001 worldlines://run-id/weights/0002

    Delete all checkpoints for a training run:

        worldlines checkpoint delete --run-id <run-id>

    Delete with filters:

        worldlines checkpoint delete --run-id <run-id> --type weights --before 2024-06-01

    Delete checkpoints in a date range:

        worldlines checkpoint delete --run-id <run-id> --after 2024-01-01 --before 2024-02-01

    Dates are interpreted as UTC. Use full ISO 8601 datetime for precision:

        worldlines checkpoint delete --run-id <run-id> --before 2024-06-01T08:00:00Z
    ...
    if not checkpoint_paths and not run_id:
        raise CyberneticsCliError(
            ...
            "Examples:\n"
            "  worldlines checkpoint delete worldlines://run-id/weights/0001\n"
            "  worldlines checkpoint delete --run-id <run-id>\n"
            "  worldlines checkpoint delete --run-id <run-id> --type weights --before 2024-06-01",
        )
    ...
    if has_filters and not run_id:
        raise CyberneticsCliError(
            ...
            "Example: worldlines checkpoint delete --run-id <run-id> --type weights --before 2024-06-01",
        )
    ...
    if run_id:
        ...
        paths_to_delete = [c.worldlines_path for c in checkpoints]
        if not yes and not _confirm_deletion(paths_to_delete, checkpoints):
            click.echo("Deletion cancelled.")
            return
    else:
        for path in checkpoint_paths:
            if not path.startswith("worldlines://"):
                raise CyberneticsCliError(
...
def download(
    ...
    Examples:
        ...
        worldlines checkpoint download worldlines://run-123/weights/final

        # Creates ./models/run-123_weights_final/ with checkpoint files
        worldlines checkpoint download worldlines://run-123/weights/final --output ./models/

        # Overwrites existing ./run-123_weights_final/ directory
        worldlines checkpoint download worldlines://run-123/weights/final --force
    """
    ...
    checkpoint_id = checkpoint_path.replace("worldlines://", "").replace("/", "_")
...
def push_hf(
    ...
    CHECKPOINT_PATH must be a worldlines path (e.g., worldlines://run-id/sampler_weights/0001).
    """
    # Validate it's a worldlines path
    if not checkpoint_path.startswith("worldlines://"):
        raise CyberneticsCliError(
            f"Invalid checkpoint path: {checkpoint_path}",
            "Checkpoint path must be in the format: worldlines://run-id/sampler_weights/0001",
        )
