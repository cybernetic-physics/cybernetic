...
from ._exceptions import APIStatusError, CyberneticsError
...
class AsyncCybernetics(AsyncAPIClient):
    # client options
    api_key: str
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async AsyncCybernetics client instance.

        This automatically infers the `api_key` argument from the
        `CYBERNETICS_API_KEY` or `CP_API_KEY` environment variable if it is not provided.
        """
        # Resolve the customer key (explicit -> CYBERNETICS_API_KEY -> CP_API_KEY
        # -> deprecated WORLDLINES_API_KEY -> stored login). The control plane validates the key;
        # the client does not assert an internal ``wld-`` prefix (that secret format
        # is a control-plane<->backend detail and must never leak to customers).
        api_key = resolve_api_key(api_key)
        if api_key is None:
            raise CyberneticsError(
                "The api_key client option must be set either by passing api_key to the client, "
                "by setting CYBERNETICS_API_KEY or CP_API_KEY, or by running "
    ...
    @cached_property
    def with_raw_response(self) -> AsyncCyberneticsWithRawResponse:
        return AsyncCyberneticsWithRawResponse(self)
    ...
    def auth_headers(self) -> dict[str, str]:
        ...
        # Bearer (not X-API-Key): the control plane's CORS allowlist accepts only
        # Authorization + Content-Type, and requireUser authenticates cp_live_ keys
        # via the Authorization header. Emitting it here also prevents a stray
        # X-API-Key from shipping alongside the default headers (double-header guard).
...
class AsyncCyberneticsWithRawResponse:
